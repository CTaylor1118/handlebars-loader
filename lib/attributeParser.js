...
AttributeContext.prototype.resolveAttributes = function(content) {
    ...
    return content.replace(regex, function(match) {
        ...
        var fallbackLoader = require.resolve('file-loader') + '?url=' + encodeURIComponent(url);
...
var processMatch = function(match, strUntilValue, name, quote, value, index) {
    var self = this;
...
var processCSSMatch = function(match, strUntilValue, quote, value, index) {
    var self = this;
    var containsTemplate = false;
    // Check if attribute is included in the "attributes" option
    if (!this.isRelevantTagAttr(this.currentTag, 'url')) {
        return;
    }
...
var specs = {
    ...
    inside: {
        ...
        "style\\s*=\\s*([\"'])?(.*?)\1": function(match, quote, css) {
            var prevTag = this.currentTag;
            this.currentTag = 'style';
            var regexp = new RegExp(cssURLRegExp);
            var results;
            while ((results = regexp.exec(css)) !== null) {
              processCSSMatch.apply(this, results);
            }
            this.currentTag = prevTag; 
            return;
        }, // Process inline styles
        "(([a-zA-Z\\-]+)\\s*=\\s*([\"'])?)([\w\/.]*)\3": processMatch
    }
};
...
styleSpec[cssURLRegExp.source] = processCSSMatch;
specs.style = styleSpec;
